    ...
    if ($for == "1") {
       ...
       if ($num > 0) {
        ...
        if ($con->query($sql1) === TRUE) {
            ...
            $getidupdate = "SELECT id_base FROM base WHERE active = '1'and department = '$department' and icd10 = '$icd10' and icd9 = '$icd9'";
            $get = mysqli_query($con, $getidupdate) or die(mysqli_error($con));
            $row = $get->fetch_assoc();
            $last_id = $row["id_base"];
